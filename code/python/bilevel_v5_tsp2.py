...
def callback(model, where):
    if where == GRB.Callback.MIPSOL:
        ...
        if len(cc) > 1:
            for comp in cc:
                if len(comp) > 1:
                    model.cbLazy(quicksum(model._x[max(a), min(a)] for a in combinations(comp, 2)) <= len(comp) - 1)
...
def getModel(G: Graph.Graph, items: list(), Lk: list(), ui, S: int, R: int, q: list(), c: float, z_bd={}):
    ...
    for i in range(1, G.n):
        x[i, 0] = model.addVar(vtype=GRB.BINARY, ub=2, name="x_%g_%g" % (i, 0))
        for j in range(1, i):
    ...
    model.addConstr(rCost == quicksum(c * G.dist[i, j] * x[i, j] for i in range(G.n) for j in range(i)))

    #balance
    model.addConstr(quicksum(x[j, 0] for j in range(1, G.n)) == 2)
    model.addConstrs(quicksum(x[i, j] for j in range(i)) + quicksum(x[j, i] for j in range(i + 1, G.n)) == 2 * g[i] for i in range(G.n))
    ...
    #duality
    model.addConstrs(quicksum(a[l] for l in Lk[k - 1]) == quicksum(items[l].ul*y[items[l].k, l] - w[items[l].k, l] + quicksum(items[l].ul*y[s + K, l] - w[s + K, l] for s in range(S)) for l in Lk[k - 1]) - quicksum(ui[k - 1][s]*p[k, s + K] for s in range(S)) + quicksum(items[l].inc * quicksum(y[s + K, l] for s in range(S)) for l in Lk[k - 1]) for k in range(1, K))
