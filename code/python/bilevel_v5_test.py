...
def getModel(G: Graph.Graph, items: list(), Lk: list(), ui, S: int, R: int, q: list(), c: float):
    ...
    #duality
    model.addConstrs(quicksum(a[l] for l in Lk[k - 1]) == quicksum(items[l].ul - w[items[l].k, l] - quicksum(w[s + K, l] for s in range(S)) for l in Lk[k - 1]) - quicksum(ui[k - 1][s]*p[k, s + K] for s in range(S)) + quicksum(items[l].inc * quicksum(y[s + K, l] for s in range(S)) for l in Lk[k - 1]) for k in range(1, K))
    ...
    model.addConstrs(a[l]  >= - items[l].price for l in range(len(items)))
    model.addConstrs(a[l] + b[s + K, l]  >= - z[s + K, l] + items[l].inc for s in range(S) for l in range(len(items)))
