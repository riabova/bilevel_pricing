...
def Bilevel_v1(G: Graph.Graph, S: int, p: float, r: dict()):
    ...
    obj = quicksum((p - z[k, i])*y[k, i] for k, i in y.keys()) - quicksum(G.dist[i, j]*x[i, j] + G.dist[i, j]*x[j, i] for i, j in G.dist.keys())
    ...
    #duality
    model.addConstrs(a[k] == (z[k, k] - r[k][0])*y[k, k] + quicksum((z[k, s + K] - r[k][s + 1])*y[k, s + K] for s in range(S)) for k in range(1, K))
